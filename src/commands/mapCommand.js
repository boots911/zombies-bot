...
// commandName is also the view key (matches a key in cards/mapConfigs.js) and
// the background image basename. mapCfg is used here only for the description.
function createMapCommand(commandName, mapCfg) {
  return {
    data: new SlashCommandBuilder()
      .setName(commandName)
      .setDescription(`Detailed ${mapCfg.name} stats for a player`)
      .addStringOption(opt =>
        opt
          .setName('username')
    ...
    async execute(interaction) {
      ...
      await sendProfile(interaction, commandName, stats);
    },
  };
}

module.exports = { createMapCommand };
